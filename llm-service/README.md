...
## Архитектура агентов и инструментов

### Декларативный подход в коде
Определения агентов и инструментов теперь находятся непосредственно в коде (а не в YAML конфигурации):

- **Агенты**: `internal/service/agent/registry.go` - содержит функцию `GetAgentsRegistry()` с определениями всех агентов
- **Инструменты**: `internal/service/tool/registry.go` - содержит функцию `GetToolsRegistry()` с определениями всех инструментов
- **Константы**: `internal/domain/agent.go` - все имена инструментов определены как константы типа `ToolName`

### Доступные агенты
1. **main** - Основной ассистент для общения с пользователем
2. **marketing_agent** - Эксперт по маркетингу и продвижению
3. **legal_agent** - Правовой консультант
4. **finance_agent** - Финансовый эксперт

### Доступные инструменты
- **Системные**: switch_to_subagent, finish_subagent
