...
export function ChatHeader({
  ...
  const usagePercent =
    limits && limits.dailyLimit
      ? Math.min(
          (((limits.used ?? 0) + (usageTokens ?? 0)) / limits.dailyLimit) * 100,
          100,
        )
      : 0;
    ...
    <Card className="relative border-none shadow-none overflow-visible rounded-b-none px-1">
      <CardHeader className="flex items-center gap-2 pb-1">
        <div className="flex flex-1 flex-col items-start gap-0.5">
          <span className="text-tiny font-medium uppercase text-default-400">
            Чат с агентом
          </span>
          <h1 className="text-base font-semibold">{chatName || "Новый чат"}</h1>
        </div>
