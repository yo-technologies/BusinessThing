...
import { CoreOrganization, CoreUserRole } from "@/api/api.core.generated";
...
export default function OrganizationsListPage() {
  ...
  const { switchOrganization } = useOrganization({ organizations, authLoading });
  const { core } = useApiClients();

  useBackButton(true);

  const { isOpen, onOpen, onClose } = useDisclosure();
  const [selectedOrg, setSelectedOrg] = useState<DetailedOrganization | null>(null);
    ...
    <div className="p-4">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0 text-left">
                  <p className="font-semibold truncate">{org.details?.name || org.id}</p>
                  <div className="flex gap-2 mt-1">
                    ...
                    {org.details?.createdAt && (
                      <Chip
                        ...
                        size="sm"
                      >
                        Создана: {new Date(org.details.createdAt).toLocaleDateString('ru-RU')}
                      </Chip>
                    )}
                  </div>
                </div>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          ...
          <ModalBody>
            <div>
              <p className="text-sm text-default-500">{selectedOrg?.details?.description || "Нет описания"}</p>
              {(selectedOrg?.details?.industry || selectedOrg?.details?.region) && (
              ...
              {selectedOrg?.details?.createdAt && (
                <p className="text-xs text-default-400 mt-1">
                  Создана: {new Date(selectedOrg.details.createdAt).toLocaleDateString('ru-RU')}
                </p>
              )}
