...
func mapDomainErrorToGRPCStatus(ctx context.Context, err error, method string) error {
	...
	logger.Errorf(ctx, "[interceptor.Error] method: %s; error: %s", method, err.Error())
...
func ErrCodesInterceptor(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (interface{}, error) {
	...
	if err != nil {
		return nil, mapDomainErrorToGRPCStatus(ctx, err, info.FullMethod)
	}
...
func ErrCodesStreamInterceptor(srv any, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
	err := handler(srv, ss)
	if err != nil {
		return mapDomainErrorToGRPCStatus(ss.Context(), err, info.FullMethod)
	}
